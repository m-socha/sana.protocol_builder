...
class MDSInstanceViewSet(viewsets.ModelViewSet):
    ...
    def get_mds_url(self, root_url, endpoint_url):
    ...
    def attempt_login(self, request):
        ...
        if mds_login_response.status_code == status.HTTP_200_OK:
            api_key = mds_login_response.json_data['api_key']
    ...
    def push_to_mds(self, request):
        ...
        mds_login_response = requests.post(
            self.get_mds_url(body['api_url'], 'push_protocol/'),
            data={
        ...
        return Response({
            'mds_status_code': mds_login_response.status_code,
        })
